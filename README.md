...
Docker 部署请查看 [DOCKER.md](./DOCKER.md)。
...
Zustand Store
  ↓
本地适配器 IndexedDB / 云端适配器 Fastify API
```

#### 本地模式

未登录时默认进入本地模式。本地模式不要求后端保存用户数据，适合离线或隐私优先使用：

- 本地上传歌曲会保存到浏览器 IndexedDB。
- 音频、封面以 Blob 形式保存，播放时生成临时 object URL。
- 歌词、收藏、生词、查词热度和学习时长都保存在 IndexedDB。
- 个人页显示本地统计，包含累计学习天数、学习时长、收藏数和生词数。
- YouTube 下载、云端同步和服务端 TTS 属于云功能，会引导用户登录。
...
登录后进入云同步模式。云同步模式复用现有 Fastify API 和 Prisma 数据模型：

- 歌曲上传、YouTube 下载、歌词编辑、生词本、收藏和学习统计写入后端。
- 多设备可以读取同一账号的数据。
- 科大讯飞 TTS 由后端代理，密钥不暴露给前端。
- YouTube 搜索和下载由后端执行，依赖代理、`yt-dlp` 和 `ffmpeg`。

#### 后续同步策略

当前实现已经把本地模式和云同步模式拆开。下一阶段可以在登录后加入同步确认流程：

1. 检测 IndexedDB 中是否存在本地歌曲、生词、收藏或学习记录。
2. 询问用户是否将本地数据同步到账号。
3. 本地歌曲上传到后端，生词、收藏和学习天数使用 upsert 合并。
4. 冲突时以后端已有数据为主，同时保留本地新增记录。
...
- 当前认证是轻量演示方案，前端通过 `localStorage` 保存会话 token；生产环境应进一步加强 Cookie、安全策略和刷新机制。
- 本地模式的数据保存在浏览器 IndexedDB 中，更换浏览器、清理站点数据或使用无痕模式会导致本地数据不可见或被清除。
- 本地模式尚未实现登录后的自动同步确认流程；目前本地数据和云端账号数据是两套独立入口。
